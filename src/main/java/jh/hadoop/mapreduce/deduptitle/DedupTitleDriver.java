 ...
 * @version 0.1
 */
public class DedupTitleDriver extends org.apache.hadoop.conf.Configured implements org.apache.hadoop.util.Tool {

    public static void main(String[] args) throws Exception {
        int res = ToolRunner.run(new DedupTitleDriver(), args);
        System.exit(res);
    }
    ...
    public int run(String[] args) throws Exception {
        ...
        job.setJobName("dedup title driver");

        job.setJarByClass(DedupTitleDriver.class);

        // Mapper & Reducer Class
        job.setMapperClass(DedupTitleMapper.class);
        // Mapper Output Key & Value Type after Hadoop 0.20
        job.setMapOutputKeyClass(Text.class);
        job.setMapOutputValueClass(NullWritable.class);

        job.setCombinerClass(DedupTitleCombiner.class);

        job.setPartitionerClass(DedupTitlePartitioner.class);


        // Reducer Output Key & Value Type
        job.setReducerClass(DedupTitleReducer.class);
